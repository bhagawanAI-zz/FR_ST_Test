...
FRVTMORPHLIB="libfrvt_morph_<company>_<three digit sequence>.so"
root=$(pwd)
approot=$root/bin
libroot=$root/lib
...
# pass into Makefile
numLibs=$(ls $libroot/libfrvt_morph_*_???.so | wc -l)
...
libstring=$(ls $libroot/libfrvt_morph_*_???.so)
